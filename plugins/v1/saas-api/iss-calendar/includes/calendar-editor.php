...
function iss_calendar_render_tag_metabox($post) {
    ...
    echo '<p><label for="iss_calendar_tag_field"><strong>Kalender-Kürzel</strong></label></p>';
    ...
    echo '<p><label for="iss_calendar_title_field"><strong>SuperSaaS-Titel</strong></label></p>';
    ...
    echo '<p class="description">Tragen Sie hier das Kürzel ein, das in SuperSaaS zu dieser Führung gehört. Beispiel: <code>ELEKTRO</code>.</p>';
    ...
    if (function_exists('iss_calendar_get_source_map_entry') && $value !== '') {
        ...
        if (is_array($entry) && !empty($entry['source_post_id'])) {
            ...
            $note = ($mapped_id === (int) $post->ID)
                ? 'Aktuell dieser Seite zugeordnet.'
                : ('Aktuell Seite #' . $mapped_id . ' zugeordnet.');
            echo '<p class="description">' . esc_html($note) . '</p>';
        }
    }

    $tools_url = admin_url('tools.php?page=iss-calendar-sync');
    echo '<p><a href="' . esc_url($tools_url) . '">Kalenderabgleich & Zuordnung öffnen</a></p>';
    ...
    if (!$show) {
        echo '<p><a href="' . esc_url(add_query_arg('iss_calendar_suggest', '1', $edit_url)) . '">Vorschläge aus SuperSaaS laden</a></p>';
        return;
    }

    if (!function_exists('iss_calendar_supersaas_fetch_free_slots') || !function_exists('is_saas_get_settings')) {
        echo '<p class="description">SuperSaaS-Vorschläge sind aktuell nicht verfügbar.</p>';
        return;
    }

    $slot_items = iss_calendar_supersaas_fetch_free_slots(is_saas_get_settings());
    if (is_wp_error($slot_items)) {
        echo '<p class="description">Vorschläge konnten nicht geladen werden.</p>';
        return;
    }
    ...
    if (empty($tags)) {
        echo '<p class="description">In den letzten SuperSaaS-Slots wurden keine Kürzel erkannt.</p>';
        return;
    }
    ...
    ksort($titles);
    echo '<p><strong>Erkannte Führungen (Titel)</strong></p><ul>';
    foreach ($titles as $t => $tagset) {
        ...
        echo '<li><strong>' . esc_html($t) . '</strong>' . $meta . ' — <a href="' . esc_url($url) . '">Übernehmen</a></li>';
    }
    echo '</ul>';
}
